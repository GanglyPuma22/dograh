...
- `setup_local.{sh,ps1}` must remain safe under unset env vars; use `${VAR:-}` guards in Bash and null/empty checks in PowerShell for optional inputs like `ENABLE_COTURN`, `TURN_HOST`, `TURN_SECRET`, `TURN_INTERNAL_HOST`, `TURN_EXTERNAL_IP`, `DOGRAH_SKIP_DOWNLOAD`.
- TURN split-host keys: `TURN_HOST` is the client-visible relay address returned by the credential endpoints. Optional `TURN_INTERNAL_HOST` is the address the API container uses to reach coturn (defaults to `TURN_HOST`). Optional `TURN_EXTERNAL_IP` is an explicit coturn `public-ip/private-ip` NAT mapping honored by the local branch of `run_dograh_init.sh` (defaults to `TURN_HOST`; the production branch stays pinned to `SERVER_IP`). Same-LAN installs need no router forwarding. `scripts/tests/test_turn_split_host.sh` covers the render and setup parity for these keys.
- `setup_local.ps1` must stay ASCII-only (like the other `.ps1` scripts): Windows PowerShell 5.1 reads BOM-less scripts as ANSI, and UTF-8 punctuation (em dashes, box-drawing, check marks) decodes into smart quotes that break parsing.
