...
| Variable | Default | Description |
|---|---|---|
| `TURN_HOST` | `localhost` | Client-visible TURN server hostname for WebRTC NAT traversal. Returned to browsers/clients in TURN credential URIs |
| `TURN_INTERNAL_HOST` | `TURN_HOST` | Optional TURN hostname used only by the server-side aiortc peer. Set it when the API container reaches coturn on a different route than clients do (e.g. `host.docker.internal` while clients use a LAN IP) |
| `TURN_EXTERNAL_IP` | `TURN_HOST` (local setup) | Optional coturn `public-ip/private-ip` NAT mapping for split-host local deployments |
