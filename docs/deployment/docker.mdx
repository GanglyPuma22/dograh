...
#### Advanced: split TURN routing

`TURN_HOST` is the client-visible address: it is what browsers and other WebRTC clients receive in their TURN credential URIs. By default the API container uses the same address to reach coturn. Two optional `.env` keys cover installs where one address cannot serve both audiences:

- `TURN_INTERNAL_HOST` — the address the API container uses to reach coturn (for example `host.docker.internal` when clients use a LAN IP the container cannot route to). Defaults to `TURN_HOST`.
- `TURN_EXTERNAL_IP` — an explicit coturn NAT mapping in `public-ip/private-ip` form (for example `192.0.2.10/172.28.0.8`) so relay candidates advertise your LAN IP while coturn stays bound to its Docker address. Defaults to `TURN_HOST`.

Preset them as environment variables when running the setup script, or add them to `.env` afterwards and recreate the `dograh-init`, `coturn`, and `api` services. Clients on the same LAN reach coturn directly, so no router port forwarding or public DNS is required.
