...
MAX_FISH_REQUEST_TIMEOUT_SECONDS = 44.0
...
class FishTTSSettings:
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "FishTTSSettings":
        ...
        request_timeout_seconds = _bounded_env_float(
            ...
            minimum=1.0,
            maximum=MAX_FISH_REQUEST_TIMEOUT_SECONDS,
        )
...
class FishTTSAdapter:
    ...
    async def synthesize(self, text: str) -> AsyncIterator[PCMChunk]:
        ...
        http_chunks = self._stream_http_chunks(payload)
        deadline = (
            asyncio.get_running_loop().time() + self.settings.request_timeout_seconds
        )
        trailing_byte = b""
        emitted_audio = False
        try:
            while True:
                try:
                    async with asyncio.timeout_at(deadline):
                        http_chunk = await anext(http_chunks)
                ...
                if complete_length == 0:
                    continue
                emitted_audio = True
                for offset in range(0, complete_length, MAX_BINARY_FRAME_BYTES):
                    yield PCMChunk(
                        audio=audio[
                            offset : min(
                                offset + MAX_BINARY_FRAME_BYTES,
                                complete_length,
                            )
                        ],
                        sample_rate=self.settings.sample_rate,
            ...
            await _close_async_resource(http_chunks)
